...
export const getDepartmentSubjects = async (req: Request, res: Response) => {
    try {
        ...
        const page = Number(normalizeQueryParam(req.query.page as string | string[] | undefined) ?? "1");
        const limit = Number(normalizeQueryParam(req.query.limit as string | string[] | undefined) ?? "10");
...
export const getDepartmentUsers = async (req: Request, res: Response) => {
    try {
        const departmentId = Number(req.params.id);
        const role = normalizeQueryParam(req.query.role as string | string[] | undefined);
        ...
        const teacherCondition: Prisma.UserWhereInput = {
            role: "teacher",
            classes: {
                some: {
                    subject: { departmentId },
        ...
        const studentCondition: Prisma.UserWhereInput = {
            role: "student",
            enrollments: {
                some: {
                    class: {
